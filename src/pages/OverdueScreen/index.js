...
export default function OverdueScreen({ navigation }) {
  ...
  async function fetchOverdueRequests() {
    ...
    try {
      ...
      setIsLoading(false);
      Alert.alert('Erro', 'Não foi possível recuperar chamados. Tente novamente!');
    }
  }

  useEffect(() => { fetchOverdueRequests() }, []);

  useEffect(() => { fetchOverdueRequests() }, [sortMode, isFocused]);
    ...
    return (
      <TouchableOpacity style={styles.sort_btn} onPress={() => handleSorting()}>
        <Text style={[styles.sub_text, { marginRight: 15 }]}>
          {sortMode === 'ASC' ? 'Mais recentes primeiro' : 'Mais antigos primeiro'}
        </Text>
  ...
  return (
    <>
      <AppHeader navigation={navigation} label="Chamados em atraso" altura="21%" />
