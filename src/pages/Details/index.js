...
export default function Details({ route, navigation }) {
  ...
  useEffect(() => {
    navigation.setOptions({
      headerRight: props => <NotesButton {...props} />,
    });
  }, [state]);
  ...
  function handleNavigateCTOMap(coordinate) {
    if (coordinate) {
      ...
      Alert.alert(
        ...
        'Este cliente não possui coordenadas definidas',
      );
    }
  }
      ...
      <ScrollView
        ...
        {refreshing !== true && (
          <>
            ...
            {state.status === 'fechado' ? <ClosingReason /> : <></>}

            <View>
              <TouchableOpacity
                onPress={() => handleNavigateCTOMap(state.coordenadas)}>
                <View style={styles.cto_line}>
